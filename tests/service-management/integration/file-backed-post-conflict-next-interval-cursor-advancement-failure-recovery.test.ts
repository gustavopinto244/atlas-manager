...
class FailFirstCursorAdvancementStore {
  public advanceCalls = 0;
  #hasFailed = false;
...
describe("file-backed post-conflict next-interval cursor-advancement failure recovery", () => {
  it("recovers through duplicate protection after post-conflict cursor-advancement rejection", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "atlas-post-conflict-cursor-advancement-failure-"),
    );
    ...
    const cursorAdvancementFailure = new Error(
      "controlled cursor advancement failure",
    );
    ...
    ).rejects.toBe(cursorAdvancementFailure);

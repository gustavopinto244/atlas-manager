...
class FailFirstClaimPruningStore {
  public pruneCompletedThroughCalls = 0;
  #hasFailed = false;

  public constructor(
    private readonly delegate: FileServiceAvailabilityReconciliationOccurrenceClaimStore,
  ...
  public pruneCompletedThrough(
    ...
    this.pruneCompletedThroughCalls += 1;
...
describe("file-backed post-conflict next-interval claim-pruning failure recovery", () => {
  it("recovers through duplicate protection after post-conflict claim-pruning rejection", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "atlas-post-conflict-claim-pruning-failure-"),
    );
    ...
    const claimPruningFailure = new Error(
      "controlled completed claim pruning failure",
    );
    const thirdStores = createStores(directory);
    const failingClaimStore = new FailFirstClaimPruningStore(
    ...
    await expect(
      third.runServiceAvailabilityReconciliationSchedulerCycle.execute(),
    ).rejects.toBe(claimPruningFailure);
    ...
    expect(failingClaimStore.pruneCompletedThroughCalls).toBe(1);

    const afterRejection = createStores(directory);
    await expect(afterRejection.cursorStore.read()).resolves.toEqual(
      secondResult.cursor,
    );
    await expect(
      afterRejection.overrideStore.findByServiceId(serviceId),
    ).resolves.toBeNull();
    await expect(
      afterRejection.claimStore.claim(secondOccurrence),
    ).resolves.toEqual({ kind: "duplicate" });
    await expect(
      afterRejection.claimStore.claim(thirdOccurrence),
    ).resolves.toEqual({ kind: "duplicate" });

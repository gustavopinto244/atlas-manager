...
describe("file-backed post-conflict next-interval cursor-conflict recovery", () => {
  it("recovers through idle after consecutive cursor conflicts", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "atlas-post-conflict-cursor-conflict-"),
    );
    ...
    expect(fourthResult.kind).toBe("idle");
    if (fourthResult.kind !== "idle") {
      throw new Error("Expected fourth scheduler cycle to return idle");
    }

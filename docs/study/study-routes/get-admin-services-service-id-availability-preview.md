...
- Diferente da leitura normal, este fluxo **não** consulta override nem status runtime — é puramente derivado da política declarada (`policy`), por isso é chamado de "preview": não reflete o estado atual, só o que a política _diria_ para aquele intervalo hipotético.

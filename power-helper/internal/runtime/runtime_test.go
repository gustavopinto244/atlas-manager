...
func TestRunReturnsOneCanonicalDenyAllResponse(t *testing.T) {
	...
	if code := Run(bytes.NewReader(request), &output, validStartup); code != 0 {
...
func TestRunRejectsWithoutOutput(t *testing.T) {
	var output bytes.Buffer
	if code := Run(bytes.NewBufferString("{}\n"), &output, validStartup); code != protocol.InvalidInputExitCode {
		t.Fatalf("unexpected exit code: %d", code)
	}
	if output.Len() != 0 {
		t.Fatalf("invalid request produced output: %q", output.String())
	}
	if code := Run(bytes.NewBufferString("{}\n"), &output, privilege.StartupDependencies{GOOS: "linux", EffectiveUID: 0, Executable: privilege.ProductionExecutablePath, ArgumentCount: 2}); code != protocol.InternalFailureExitCode {
